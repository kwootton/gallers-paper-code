...
compare_predictions_fig <- function(prb, data, res, cons, 
                                    showpredict = TRUE, legpos = "right", 
                                    plottitle = "", group_res_by=NA, group_con_by=NA, 
                                    showcoexist = FALSE, flip = FALSE){
  ...
  if(length(group_res_by) > 1 & length(group_con_by) > 1 & flip == TRUE)
    fg <- facet_grid(rows = vars(grouping_factor_con), cols = vars(grouping_factor_res), scales = "free", space = "free") 
  if(length(group_res_by) > 1 & length(group_con_by) > 1 & flip == FALSE)
  ...
  if(length(group_res_by) == 1 & length(group_con_by) == 1)
    fg <- NA
...
make_sp_names_table <- function(sp, group_res_by=NA){
  
  if(sp == "Salix"){
    sp_names <- rownames(salixGal$adjMat)
    df_salix <- readRDS("original.data.from.paper/df_salix.rds")
    salix_names <- paste("Salix",df_salix$SPECIES[match(unlist(sp_names), df_salix$RSAL)])
    
    stargazer::stargazer(data.frame("Salix species" = salix_names), summary = FALSE, rownames = FALSE)
  }
  
  if(sp == "Galler"){
    group_res_by <- group_res_by[order(group_res_by$grouping_factor),]
    df_galler <- readRDS("original.data.from.paper/df_galler.rds")
    group_res_by$`Galler species` <- df_galler$GENUS_SPECIES[match(unlist(group_res_by$RGALLER), df_galler$RGALLER)]
    group_res_by$`Gall type` <- df_galler$`CODE_GALLTYPE SIMPLIFIED`[match(unlist(group_res_by$RGALLER), df_galler$RGALLER)]
    df <- data.frame("Galler species" = group_res_by$`Galler species`,"Gall type" = group_res_by$`Gall type`, "Gall type abbreviation" = group_res_by$grouping_factor)
    
    stargazer::stargazer(df, summary = FALSE, rownames = FALSE)
  }
  
  if(sp == "Parasitoid"){
    sp_names <- colnames(galPara$adjMat)
    df_para <- readRDS("original.data.from.paper/df_parasit.rds")
    paras_full_names <- df_para$FULL_NAME[match(unlist(sp_names), df_para$RPAR)]
    
    stargazer::stargazer(data.frame("Parasitoid species" = paras_full_names), summary = FALSE, rownames = FALSE)
